...
let restaurants = [
    { name: "Burger House", category: "comida rapida", rating: 4.5, price: "Moderado", location: "Centro", delivery: 25, description: "Hamburguesas rápidas y deliciosas", _id: "1" },
    { name: "Sushi King", category: "sushi", rating: 4.8, price: "Caro", location: "Zona Norte", delivery: 40, description: "Sushi fresco y premium", _id: "2" },
    { name: "Veggie Delight", category: "vegetariano", rating: 4.6, price: "Moderado", location: "Zona Sur", delivery: 30, description: "Opciones saludables y vegetarianas", _id: "3" }
];
...
document.getElementById('restaurantForm').addEventListener('submit', async function (e) {
    ...
    try {
        if (currentId) {
            ...
            restaurants = restaurants.map(r => r._id === currentId ? { ...formData, _id: currentId } : r);
...
function filterTable() {
    ...
    document.querySelectorAll('#tableBody tr').forEach(r => {
        ...
        const rating = parseFloat(r.cells[2]?.textContent.match(/[\d.]+/)?.[0]) || 0;
        r.style.display = (name.includes(search) && (!cat || category === cat) && (!rate || rating >= rate)) ? '' : 'none';
    });
}
...
function showNotification(msg, type = 'success') {
    ...
    n.style.cssText = `position:fixed;top:100px;right:20px;background:${type === 'success' ? '#4caf50' : '#f44336'};color:white;padding:1rem 1.5rem;border-radius:10px;box-shadow:0 4px 15px rgba(0,0,0,0.3);z-index:10000;font-weight:600;animation:slideIn 0.3s ease;`;
    n.textContent = msg;
